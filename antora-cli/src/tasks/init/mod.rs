...
fn validate_project_init_dir_or_exit(project_init_dir: Option<&String>) -> ProjectInitDir {
    ...
    match ProjectInitDir::try_from(project_init_dir) {
        ...
        Err(AntoraFsError::InitDirContainsDefaultPlaybook) => {
            eprintln!("the provided project-dir contains a default playbook and cannot be used.");
            exit(1);
        }
        Err(e) => {
...
fn check_for_conflicts_persisting_or_exitting(vfs: &Vfs) -> u64 {
    ...
    if conflicts.is_empty() {
        ...
        eprintln!("aborting due to conflicts ; since the following files would be overwritten:");
...
pub fn run(init_args: InitArgs) {
    ...
    let project_init_dir = validate_project_init_dir_or_exit(init_args.project_dir);

    // check for invalid git-constellation
    let create_git_repo = check_git_constellation_exit_if_nested(&project_init_dir);
    ...
    check_or_create_secrets_exit_if_encountered_invalid(&project_init_dir, &vfs);
    ...
    let component_version = if let Some(component_version) = init_args.provided_component_version {
    ...
    let mut template =
        Templates::Basic.create(&results, component_version, init_args.include_scaffolding);
    ...
    check_for_conflicts_persisting_or_exitting(&vfs);

    // create a git-repo if ok according to previous git-analysis
    if create_git_repo {
        create_git_repo_or_exit(project_init_dir.as_ref(), "initial commit");
    }
}
